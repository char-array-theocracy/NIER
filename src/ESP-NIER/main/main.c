...
#define BROKER_URI "mqtts://192.168.1.113:8883"
...
static void smartSwitch(void *pvParamaters)
{
    ...
    while (1) {
        ...
        if (switchResponse) {
            cJSON_AddNumberToObject(switchResponse, "time", time(NULL));
...
static void temperatureHumidityTask(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (aht10_readData(&temperature, &humidity) == ESP_OK) {
            cJSON *message = cJSON_CreateObject();
            if (message) {
                cJSON_AddNumberToObject(message, "time", time(NULL));
                cJSON_AddNumberToObject(message, "temperature", temperature);
                cJSON_AddNumberToObject(message, "humidity", humidity);
                char *jsonString = cJSON_PrintUnformatted(message);
                if (jsonString) {
                    char topic[128];
